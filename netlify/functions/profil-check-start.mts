...
import { STORE_NAME, checkAccessCode, isConfigured, json } from "../lib/common.mts";
...
export default async (req: Request, _context: Context) => {
  ...
  let body: { accessCode?: unknown; handle?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const res = await fetch(new URL("/.netlify/functions/profil-check-background", req.url), {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ id, accessCode: body.accessCode, profile }),
  });
